...
namespace WinFormUI
{
    public partial class Dashboard : Form
    {
        ...
        {
            decimal total = cart.GenerateTotal(SubTotalAlert, CalculateLevelledDiscount, PrintOutDiscountAlert);
        ...
        {
            decimal total = cart.GenerateTotal((subTotal) => subTotalTextBox.Text = $"{subTotal:C2}", (products, subTotal) => subTotal - (products.Count * 2), (message) => { });
        ...
        private decimal CalculateLevelledDiscount(List<ProductModel> products, decimal subtotal)
        {
            if (products.Count > 3)
            {
